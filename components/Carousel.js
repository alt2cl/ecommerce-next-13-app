...
function Carousel({arrayImage, passId}) {
    ...
    const carouselItems = arrayImage.map(img => 
        (
            <div className="list-item-carousel" style={{minWidth: (widthwrapcarousel)}} key={img.attributes.url} >
    ...
    const nextCarousel = (( direccion, widthItem) => {
        ...
        console.log('clickeado r max r', maxRight, widthWrapperCarousel)

        if(direccion === 'r') {
            //positionCarousel = positionCarousel - widthItem //0-409= -409
            console.log('clickeado r', positionCarousel)

            if(positionCarousel <= 0 && positionCarousel >= ((maxRight - (widthItem  * carouselItems.length)) * -1)){
                wrapcarousel.style.left = positionCarousel - widthItem + 'px'; //debe dar -315

                setPositionCarousel(positionCarousel - widthItem)

                console.log('clickeado r yes', positionCarousel)
                
            } else {

                console.log('clickeado r no', positionCarousel)
                wrapcarousel.style.left = '0px',
                setPositionCarousel(0)
            }
        } else if (direccion === 'l'){

            console.log('clickeado l')
    ...
    return (
        ...
        <div className="wrap-control-carousel">
            <div className="control-carousel">
                <a className="left-control controlbtn text-black" onClick={() => nextCarousel( 'l', widthwrapcarousel)}>
                ...
                <a className="left-control controlbtn text-black" onClick={() => nextCarousel( 'r', widthwrapcarousel)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="black" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
        ...
        <div className="results-list-container results-list-carousel-container" id={passId}>
            {carouselItems}
        </div>
    </div>
    );
