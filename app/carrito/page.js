...
export default function CarritoPage(props) {
    ...
    const preferenceObject = {
        ...
        "image":"https://fastly.picsum.photos/id/277/200/300.jpg?hmac=8wIsJ0ZOFtBSrTpSE0M37XZG2dXMzDTL2_dgppOaP0Y",
        "price":990
    }
    ...
    useEffect(()=> {
        const calculoTotal = carrito.reduce((totalams, producto) => totalams + (producto.cantidad * producto.precio), 0)
            ...
            <div className="grid lg:grid-cols-12 lg:gap-12">
                    ...
                    <div>
                        {carrito.length === 0 ? 'Carrito vacio' : (
                            carrito.map(producto=>(
                                <div key={producto.id} className='flex bg-white mb-4 relative rounded'>
                                    <div>
                                        <Image 
                                        src={producto.imagen} 
                                        width={200} 
                                    ...
                                    <div className='flex-1 p-3'>
                                        <h4 className="text-md text-slate-700 font-semibold mb-2">{producto.nombre}</h4>
                                        <p className="text-lg text-slate-800 font-semibold mb-4">Precio: ${producto.precio}</p>
                                        <div className="flex flex-row items-center justify-between pr-5">
                                            <div className="cantidad-select mr-2 rounded-full bg-slate-300 px-3 py-2 ">
                                                
                                                <label htmlFor="" className="mr-3">Cantidad</label>
                                            ...
                                            <p className=" text-lg font-semibold text-slate-800">Subtotal: $ {producto.cantidad * producto.precio}</p>
                ...
                <aside className="block col-span-12 lg:col-span-5">
                    <div className="mb-5 p-4 bg-white">
                        ...
                        <Link href={'/carrito/envio'} className="rounded text-center bg-yellow-400 p-2 block w-full">Continuar compra</Link>
                    ...
                    <div className="mb-5 p-4 bg-white">
                        ...
                        <button id="checkout-btn" onClick={()=>{
                            axios.post('http://localhost:3001/payment', preferenceObject ).then((res)=> window.location.href = res.data.response.body.init_point)
                        }}>Checkout</button>
                    ...
                    <div className="mb-5 p-4 bg-white">
                        <h5 className="text-lg font-semibold mb-3">Métodos de envio y costos</h5>
