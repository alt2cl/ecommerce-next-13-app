...
async function fetchNosotros() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_STRAPI_URL}/companies?populate=contentblock`,
    { next: { revalidate: 60 } }
  );
  return res.json();
}
...
export default async function NosotrosPage(props) {
  ...
  const secciones = await Promise.all(
    data.map(async (content, i) => {
      const richtext = await markdownToHtml(
        content.attributes.contentblock[i].content
      );
      ...
      return await (
        <section
          ...
          style={{
            backgroundColor: content.attributes.contentblock[i].background
              ? "#f0ece7"
              : "",
            backgroundImage: content.attributes.contentblock[i].background
              ...
              : null,
            padding: content.attributes.contentblock[i].background
              ? "30px"
              : "",
          }}
        >
          ...
          {content.attributes.contentblock.map((item) => {
            return (
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
                <div className="col-span-2">
                  {content.attributes.imagen?.data && (
                    <Image
                      src={
                        content.attributes.imagen?.data.attributes.formats
                          ?.medium
                          ? content.attributes.imagen?.data.attributes.formats
                              .medium.url
  ...
  return (
    <div className="container px-4 pt-9">
      <main className="mb-8">{secciones}</main>
    </div>
  );
}
