...
async function fetchPosts (){
    let res = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_URL}/productos?populate=*`)
    return res.json();
}
            ...
            <>
                {
                    data.map(post=>{
                        ...
                        const {titulo, descripcion, publishedAt, url, imagen, precio, categorias} = item
                        ...
                        <div key={post.id} className={'flex flex-col gap-3 mb-5 lg:mb-16'} >
                            ...
                            <div className='flex flex-col col-span-4'>
                                <h3 className="text-md mb-3 font-semibold text-slate-900">{titulo}</h3>
                                ...
                                <Link href={`/tienda/${categorias.data[0].attributes.titulo}/${url}`} className="rounded-md text-center p-2 bg-orange-500 hover:bg-slate-900 text-white">
                                    Ir al producto...
                                </Link>
                            </div>
                        </div>
